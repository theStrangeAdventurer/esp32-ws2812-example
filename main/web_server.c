...
static esp_err_t status_get_handler(httpd_req_t *req) {
    ...
    if (ret == ESP_OK) {
        ...
        cJSON_AddNumberToObject(json, "current_effect_index", status.current_effect);
        cJSON_AddNumberToObject(json, "total_effects", status.total_effects);
        cJSON_AddNumberToObject(json, "brightness", effect_manager_get_brightness(g_effect_manager));
        cJSON_AddBoolToObject(json, "is_running", g_effect_manager->params->running);
...
static esp_err_t effect_post_handler(httpd_req_t *req) {
    ...
    if (cJSON_IsString(effect)) {
        ...
        err = effect_manager_set_effect_by_name(g_effect_manager, effect->valuestring);
    ...
    if (err == ESP_OK) {
        ...
        cJSON_AddStringToObject(response, "current_effect", effect_manager_get_current_name(g_effect_manager));
        ...
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Invalid effect name or index");
    }
...
static esp_err_t next_effect_handler(httpd_req_t *req) {
    ...
    if (err == ESP_OK) {
        ...
        cJSON_AddStringToObject(response, "status", "success");
        cJSON_AddStringToObject(response, "current_effect", effect_manager_get_current_name(g_effect_manager));
        cJSON_AddNumberToObject(response, "current_index", effect_manager_get_current_index(g_effect_manager));
        ...
        httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Failed to switch effect");
    }
...
static esp_err_t brightness_post_handler(httpd_req_t *req) {
    ...
    if (err == ESP_OK) {
        ...
        httpd_resp_send_err(req, HTTPD_400_BAD_REQUEST, "Invalid brightness value or delta");
    }
...
static esp_err_t root_handler(httpd_req_t *req) {
    ...
    "<div class='control-group'>"
    "<h3>Effects</h3>"
    "<div id='effectButtons' class='effect-buttons'>Loading effects...</div>"
    "<br>"
    ...
    "<input type='range' id='brightnessSlider' class='slider' min='1' max='255' value='128' onchange='setBrightness(this.value)'>"
    "<button onclick='adjustBrightness(20)'>+</button>"
    ...
    "    `<strong>Current Effect:</strong> ${status.current_effect || 'Unknown'}<br>`+"
    "    `<strong>Brightness:</strong> ${status.brightness || 0}<br>`+"
    "    `<strong>Running:</strong> ${status.is_running ? 'Yes' : 'No'}<br>`+"
    "    `<strong>Total Effects:</strong> ${status.total_effects || 0}`;"
    "  "
